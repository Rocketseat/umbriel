  ...
  const handleDelete = useCallback(async (id: string) => {
    ...
    if (confirmed) {
      try {
        ...
        mutate(request.requestKey, { ...request.response, data: request.data?.filter(message => message._id !== id) }, false);
      } catch (err) {
        ...
        <tbody>
          {messages.map(message => (
            <Fragment key={message._id}>
              <tr>
                <td colSpan={5}><h3>{message.subject}</h3></td>
              </tr>
              <tr className="no-border">
                <td style={{ width: 200 }}>
                  ...
                  >
                    <span>{message.sentCount} / {message.recipientsCount}</span>
                  </ProgressBar>
                </td>
                <td>
                  {message.visualTags}
                  <small>{message.tags.length > 2 ? ` e outras ${message.tags.length - 2}` : ''}</small>
                </td>
