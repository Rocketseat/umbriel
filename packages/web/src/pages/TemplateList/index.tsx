...
const Templates: React.FC<Props> = ({ history }) => {
  ...
  const handleDelete = useCallback(async (id: string) => {
    const confirmed = window.confirm('Deseja realmente deletar essa template? Olha lá o que tu vai fazer...');

    if (confirmed) {
      try {
        await axios.delete(`/templates/${id}`)

        mutate(request.requestKey, { ...request.response, data: request.data?.filter(template => template._id !== id) }, false);
      } catch (err) {
