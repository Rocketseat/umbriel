...
import useRequest, { Return } from '../services/useRequest';
...
export default function usePaginatedRequest<Data = unknown, Error = unknown>(request: AxiosRequestConfig): PaginatedRequest<Data, Error> {
  ...
  const hasNextPage = useMemo(() => page < response?.headers['x-total-page'], [page, response]);
